  ...
  return (
    {
      ...
      token_exchange_failed:
        "Google could not complete the Google Drive connection. Please try again.",
      account_lookup_failed:
        "ReadiNes could not read the selected Google account. Please try another account.",
      missing_refresh_token:
        "Google did not return offline access. Remove ReadiNes from Google Account connections, then reconnect.",
      missing_scope:
        "Google Drive read-only permission was not granted. Please reconnect and allow it.",
      database_error:
        "ReadiNes could not save the Google Drive connection. Please try again.",
      token_storage_failed:
        "ReadiNes could not securely store the Google Drive connection. Please try again.",
    }[payload.reason ?? ""] ??
    "Google Drive connection was not completed. Please try again."
  );
}
...
export default function DriveDialog({
  ...
  const scan = useCallback(
    async (full = false, currentStatus = status) => {
      setBusy("scan");
      setMessage("");
      updateStatus({
        ...currentStatus,
      ...
      try {
        ...
        setMessage(
          next.error ||
            `${result.message} Imported ${result.imported}, skipped ${result.skipped}, failed ${result.failed}.`,
        );
      } catch (error) {
        setBusy("");
        updateStatus({
        ...
        setMessage(
          error instanceof Error
            ? error.message
            : "Google Drive scan could not start. Reconnect if access was revoked.",
        );
      }
    },
  ...
  useEffect(() => {
    if (open)
      void refresh().catch(() =>
        setMessage("Unable to load Google Drive connection."),
      );
  }, [open, refresh]);
  useEffect(() => {
  ...
  const driveOAuth = useGoogleOAuthPopup({
    provider: "drive",
    popupName: "ReadiNes-drive-oauth",
  ...
  const connect = async () => {
    ...
    try {
      ...
      if (!driveOAuth.openPopup(authorizationUrl)) {
        setBusy("");
        setMessage("Allow popups to connect Google Drive.");
      }
    } catch {
      setBusy("");
      setMessage("Unable to start Google Drive authorization.");
    }
  };
  const disconnect = async () => {
    if (
      !confirm(
        "Disconnect Google Drive? Indexed metadata will remain in ReadiNes.",
      )
    )
      return;
    setBusy("disconnect");
    try {
      ...
      setMessage("Unable to disconnect Google Drive.");
    } finally {
      setBusy("");
    }
  };
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: "min(650px,calc(100vw - 28px))" }}
      >
        <div
          style={{ display: "flex", justifyContent: "space-between", gap: 12 }}
        >
          <div>
            <h2 id="drive-title" style={{ margin: 0, color: T.white }}>
              Google Drive
            </h2>
            <p style={{ color: T.muted, fontSize: 13 }}>
              Read-only PDF discovery and indexing
            </p>
          </div>
          <button
        ...
        {!status.connected ? (
          <div style={{ padding: 16, background: T.raised, borderRadius: 12 }}>
            <div style={{ display: "flex", gap: 12 }}>
              <Cloud color={T.gold} />
              <div>
                <b style={{ color: T.white }}>Google Drive is not connected</b>
                <p style={{ color: T.text, fontSize: 13, lineHeight: 1.6 }}>
                  ReadiNes requests only Drive read-only access, searches only
                  for PDFs, and stores metadata—not PDF copies.
                </p>
              </div>
            </div>
            <button
          ...
          <div style={{ padding: 16, background: T.raised, borderRadius: 12 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <Check color={T.mint} />
              <div>
                <b style={{ color: T.white }}>Connected</b>
            ...
            >
              ...
              <div style={{ color: T.muted, fontSize: 12 }}>
                PDFs Indexed
                <br />
                <b style={{ color: T.white }}>{status.indexedCount}</b>
              </div>
            </div>
            {status.scanning ? (
              <div style={{ marginTop: 18 }}>
                <div style={{ color: T.gold, fontWeight: 700 }}>
                  {status.phase || "Loading..."}
                </div>
                <div style={{ color: T.white, marginTop: 6 }}>
                  {status.processed} / {status.total} PDFs
                </div>
                <progress
            ...
            >
              <button
                style={btnGold}
                disabled={Boolean(busy) || status.scanning}
            ...
            >
              ...
              <select
                ...
                onChange={(event) =>
                  setDuplicateAction(
                    event.target.value as typeof duplicateAction,
                  )
                }
                style={{
                  padding: "9px 10px",
        ...
        {message || status.error ? (
          <div
            style={{
              color: status.error ? T.coral : T.muted,
              fontSize: 12,
              marginTop: 12,
            }}
          >
