...
export default function UploadDocumentModal({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved?: (document: DocumentRecord) => void }) {
  ...
  return <div className="lp-modal-backdrop" style={{ zIndex: 50 }}><div className="lp-modal-panel" style={{ width: "min(720px, calc(100vw - 36px))", maxHeight: "86vh", overflow: "auto" }}>
    ...
    {!pendingAnalysis ? <div data-testid="document-drop-zone" onDragOver={(event) => event.preventDefault()} onDrop={onDrop} style={{ border: `1px dashed ${T.border}`, borderRadius: 12, padding: 20 }}>
      ...
      <div style={{ color: T.muted, fontSize: 12, marginTop: 8 }}>JPG, PNG, WebP, or PDF · up to 10 files</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(130px,1fr))", gap: 10, marginTop: 14 }}>{previews.map(({ file, url }, index) => <div key={`${file.name}-${file.size}`}>{isPdf(file) ? <div style={{ height: 100, display: "grid", placeItems: "center", padding: 10, border: `1px solid ${T.border}`, borderRadius: 8, background: T.raised, color: T.white, fontWeight: 800, textAlign: "center", wordBreak: "break-word" }}>PDF<br /><small style={{ color: T.muted, fontWeight: 600 }}>{file.name}</small></div> : <img src={url} alt={`Page ${index + 1}`} style={{ width: "100%", height: 100, objectFit: "contain", background: T.raised }} />}<button style={btnGhost} onClick={() => setFiles((items) => items.filter((_, i) => i !== index))}>Remove</button></div>)}</div>
